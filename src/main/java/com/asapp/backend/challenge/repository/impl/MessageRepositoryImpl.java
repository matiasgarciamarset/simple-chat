...
public class MessageRepositoryImpl extends GenericDao<Message, Integer> implements MessageRepository {
    ...
    public List<Message> listMessages(User sender, User recipient, Integer start, Integer limit) {
        ...
        return session.createQuery("from Message where sender = :value1 and recipient = :value2", Message.class)
                .setParameter("value1", sender)
                .setParameter("value2", recipient)
                .setMaxResults(limit)
                .setFirstResult(start > 0 ? start-1 : start)
                .list();
    }
}
